            ...
            <main className="relative mx-auto flex w-full max-w-5xl min-h-0 flex-1 flex-col px-5 pb-10 pt-10 sm:px-8 sm:pb-12 lg:px-12">
              <header className="relative z-10 mb-8 flex flex-wrap items-end justify-between gap-4 border-b border-border/50 pb-8">
                <div className="min-w-0 max-w-2xl">
                  ...
                  <p className="truncate text-[13px] leading-relaxed text-muted-foreground sm:text-sm">
                    {project?.repoFullName ?? "Repository"} · {project?.sandboxId ?? project?.sandboxStatus ?? "loading"}
                  </p>
                  <p className="mt-3 text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
                    Thread
                  </p>
                  <h1
                    className={`${display.className} mt-2 truncate text-[clamp(1.3rem,3.5vw,1.8rem)] font-extrabold uppercase leading-[1.08] tracking-[0.04em]`}
                  >
