...
function PullRow({ pull, selected, onSelect }: { pull: ProjectPullRequest; selected: boolean; onSelect: () => void }) {
  return (
    ...
    >
      <PullRequestStatus state={pullRequestState(pull)} />
    ...
    <div>
      {Array.from({ length: 7 }).map((_, index) => (
        <div key={index} className="flex items-start gap-3 border-b border-border/60 px-3 py-3">
          <Skeleton className="animate-none size-4 rounded-xs" />
          <div className="min-w-0 flex-1 space-y-2"><Skeleton className="animate-none h-3 w-4/5 rounded-xs" /><Skeleton className="animate-none h-2.5 w-3/5 rounded-xs" /></div>
          <Skeleton className="animate-none h-3 w-7 rounded-xs" />
        </div>
      ))}
    </div>
  );
...
function PullList({
  ...
  const visible = useMemo(() => {
    ...
    return pulls.filter((pull) => {
      if (filter !== "all" && pullRequestState(pull) !== filter) return false;
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="shrink-0 border-b border-border p-3">
        ...
        <div className="mt-2 flex items-center gap-1 overflow-x-auto" aria-label="Filter pull requests">
          {(["all", "open", "draft", "merged", "closed"] as const).map((item) => (
            <button key={item} type="button" aria-pressed={filter === item} onClick={() => onFilter(item)} className={cn("inline-flex h-7 shrink-0 items-center gap-1.5 border-b-2 border-transparent px-2 text-[11px]", filter === item ? "border-white text-white" : "text-muted-foreground hover:bg-[color:var(--project-panel-soft)] hover:text-foreground")}>
...
export function PullRequestWorkspace({
  ...
  const refresh = () => {
    void queryClient.invalidateQueries({ queryKey: ["project", projectId, "pulls"] });
    if (selectedNumber !== undefined) {
      void queryClient.invalidateQueries({ queryKey: ["project", projectId, "pull", selectedNumber] });
    }
  };
  ...
  return (
    <div className={cn("flex h-full min-h-0 flex-col bg-black text-white [--background:#000] [--foreground:#fff] [--card:#000]", variant === "page" && "pull-request-workspace")}>
      <header className="flex min-h-11 shrink-0 items-center gap-2 border-b border-border px-3 py-2">
        <span className="inline-flex size-5 shrink-0 items-center justify-center"><GitPullRequest className="size-4 text-foreground/80" aria-hidden="true" /></span>
        <div className="min-w-0 flex-1 flex flex-wrap items-baseline gap-x-2">
          ...
          <p className="mt-0.5 truncate font-mono text-[10px] text-muted-foreground">{listQuery.data?.project.repoFullName ?? (listQuery.isPending ? "Loading repository…" : "Repository unavailable")}</p>
        </div>
        {refreshing ? <span role="status" className="text-[10px] text-muted-foreground">Refreshing…</span> : null}
        <button type="button" onClick={refresh} disabled={refreshing} className="inline-flex size-8 items-center justify-center border border-border text-muted-foreground hover:bg-[color:var(--project-panel-soft)] hover:text-foreground disabled:opacity-50" aria-label="Refresh pull requests"><RefreshCw className="size-3.5" aria-hidden="true" /></button>
