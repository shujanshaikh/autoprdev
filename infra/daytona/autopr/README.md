...
- The web preview trusts noVNC's completed connection handshake instead of treating dark framebuffer pixels as a transport failure. A failed handshake pauses browser retries while the backend repairs the route. Recovery keeps a reachable x11vnc session alive, replaces noVNC, waits for ports 5901 and 6080, and verifies the signed public preview URL before returning it to the browser. Signed token paths are preserved when `/websockify` is appended. Failed recoveries continue with capped backoff instead of exhausting a fixed retry budget. The compact and full viewers share preview requests and one five-minute activity heartbeat per project, keeping an actively viewed desktop from falling through the 15-minute auto-stop window without duplicate Daytona calls.
