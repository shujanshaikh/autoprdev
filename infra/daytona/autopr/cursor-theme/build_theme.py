#!/usr/bin/env python3
# Derived from CUA Driver 0.19.3 (MIT): https://github.com/trycua/cua
"""Build AutoPR's deterministic CUA dotLottie cursor theme.

Derived from CUA Driver's ``cua.default`` generator at release 0.19.3. This
script deliberately uses only Python's standard library so contributors can
reproduce the source archive without a LottieFiles account or browser editor.

The runtime applies two bounded effects after decoding the compiled theme:

* the theme keeps authored violet, cyan, and white colors;
* the shared four-second float transform is applied by CUA at runtime.

Delivery and target context remains rendered by CUA's host-owned badge.
"""
...
CANVAS = 128
FPS = 30
PRIMARY = [139 / 255, 92 / 255, 246 / 255, 1]
GLOW = [192 / 255, 132 / 255, 252 / 255, 1]
ACCENT = [34 / 255, 211 / 255, 238 / 255, 1]
WHITE = [1, 1, 1, 1]
TRANSPARENT = [0, 0, 0, 0]
    ...
    *,
    ...
    for width, opacity in [
        ...
        layers.append(
            shape_layer(
                index,
                f"Cursor glow {width}",
                [CURSOR_PATH, stroke(GLOW, width, opacity)],
                frames,
                position=position,
                scale=scale,
            )
    ...
    layers.append(
        shape_layer(
            index,
            "Cursor body",
            [CURSOR_PATH, fill(PRIMARY), stroke(WHITE, 5)],
            frames,
            position=position,
            scale=scale,
        )
...
def action_record() -> dict[str, Any]:
    ...
    dot = [
        shape_layer(
            30,
            "Record dot outline",
            [*dot_geometry, fill(ACCENT), stroke(WHITE, 3)],
            frames,
            position=static([52, 64]),
            scale=dot_scale,
            opacity=dot_opacity,
...
def semantic_manifest() -> dict[str, Any]:
    ...
    return {
        ...
        "id": "dev.autopr.cursor.neon",
        "name": "AutoPR Neon",
        "version": "2.0.0",
        "author": "AutoPR, derived from CUA Driver",
        "license": "MIT",
...
def build(output: Path) -> None:
    ...
    standard_manifest = {
        "version": "2",
        "generator": "AutoPR neon cursor generator (derived from CUA Driver 0.19.3)",
        "animations": [
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).with_name("autopr-neon.lottie"),
    )
