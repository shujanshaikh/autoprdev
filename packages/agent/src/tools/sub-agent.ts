...
export function createSubAgentTool(options: SubAgentToolOptions) {
  ...
  return tool({
    ...
    description:
      ...
      `The sub-agent shares the selected sandbox workspace and authenticated model with its parent, receives only the supplied prompt, and cannot delegate again. ` +
